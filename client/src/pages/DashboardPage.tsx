  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white text-xs sm:text-sm font-bold shadow-lg shadow-blue-600/30 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer"
            >
              <CheckSquare className="w-4 h-4" /> {isMember ? 'Xem Việc Của Tôi' : user?.role === 'SECRETARY' ? 'Theo Dõi Công Việc' : 'Giao Việc Mới'}
            </button>
            <button
              onClick={() => setActiveTab('meetings')}
              className="flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white text-xs sm:text-sm font-bold shadow-lg shadow-indigo-600/30 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer"
            >
      ...
      {pendingAcceptTasks.length > 0 && (
        <div
          className={`p-5 rounded-3xl border shadow-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-in slide-in-from-top-2 ${
            isLight
        ...
        >
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h4 className={`text-sm font-bold ${isLight ? 'text-purple-950' : 'text-white'}`}>
                Bạn có {pendingAcceptTasks.length} công việc mới được giao đang chờ tiếp nhận!
              </h4>
              <p className={`text-xs ${isLight ? 'text-purple-800' : 'text-purple-300'}`}>
          ...
          <div className="flex items-center gap-2">
            {pendingAcceptTasks.map((t: any) => (
              <button
                ...
                className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-xl text-xs font-bold shadow-md shadow-purple-600/30 transition flex items-center gap-1.5 cursor-pointer"
              >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
        ...
        <div className="xl:col-span-7 space-y-8">
          {/* Recent Tasks Card */}
          <div className="glass-panel p-6 sm:p-7 rounded-3xl space-y-5 shadow-xl">
            <div className={`flex items-center justify-between pb-3 border-b ${isLight ? 'border-slate-200' : 'border-slate-800'}`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className={`text-base sm:text-lg font-bold ${isLight ? 'text-slate-900' : 'text-white'}`}>
                    {isMember ? 'Công Việc Của Bạn Hôm Nay' : 'Tiến Độ Công Việc Gần Đây'}
                  </h3>
              ...
              <button
                onClick={() => setActiveTab('tasks')}
                className={`text-xs font-bold flex items-center gap-1.5 px-3 py-1.5 rounded-xl transition cursor-pointer border ${
                  isLight
            ...
            <div className="space-y-3.5">
              {tasks.recent?.length === 0 ? (
                <div
                  className={`text-center py-12 space-y-3 rounded-2xl border ${
                    isLight ? 'bg-slate-50 border-slate-200' : 'bg-slate-900/40 border-slate-800'
                  }`}
                >
                  <CheckSquare className={`w-8 h-8 mx-auto ${isLight ? 'text-slate-400' : 'text-slate-600'}`} />
                  <p className={`text-xs ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>
                ...
                tasks.recent?.map((t: any) => (
                  <div
                    key={t.id}
                    onClick={() => setActiveTab('tasks')}
                    className={`p-4 sm:p-5 rounded-2xl transition-all duration-200 cursor-pointer space-y-3 shadow-sm hover:shadow-md group border ${
                      isLight
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span
                        className={`text-[10px] font-extrabold px-2.5 py-1 rounded-full whitespace-nowrap border ${
                          t.status === 'DONE'
                    ...
                    <div className="space-y-1.5">
                      <div className={`flex justify-between text-xs ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>
                        <span className="font-medium">Tiến độ việc con ({t.completedSubtasks || 0}/{t.totalSubtasks || 0})</span>
                        <span className="font-extrabold text-blue-600 dark:text-blue-400">{t.progress}%</span>
                      </div>
                      <div className={`w-full h-2 rounded-full overflow-hidden p-0.5 border ${isLight ? 'bg-slate-200 border-slate-300' : 'bg-slate-800 border-slate-700/50'}`}>
                        <div
          ...
          <div className="glass-panel p-6 sm:p-7 rounded-3xl space-y-5 shadow-xl">
            <div className={`flex items-center justify-between pb-3 border-b ${isLight ? 'border-slate-200' : 'border-slate-800'}`}>
              <div className="flex items-center gap-3">
                <div
                  className={`p-2.5 rounded-xl border ${
                    isLight ? 'bg-purple-100 text-purple-700 border-purple-200' : 'bg-purple-500/15 text-purple-400 border border-purple-500/20'
                  }`}
                >
              ...
              >
                Xem lịch <ArrowUpRight className="w-3.5 h-3.5" />
              </button>
            </div>
            ...
            <div className="space-y-3.5">
              {meetings.today?.length === 0 ? (
                ...
                >
                  <Calendar className={`w-8 h-8 mx-auto ${isLight ? 'text-slate-400' : 'text-slate-600'}`} />
                ...
                meetings.today?.map((m: any) => (
                  <div
                    key={m.id}
                    className={`p-4 rounded-2xl transition space-y-2 border ${
                      isLight ? 'bg-slate-50/80 border-slate-200' : 'bg-slate-900/60 border-slate-800/80'
                    }`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <p className={`font-bold text-xs sm:text-sm ${isLight ? 'text-slate-900' : 'text-white'}`}>{m.title}</p>
                      <span
                    ...
                    <div className={`flex items-center gap-4 text-xs ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>
