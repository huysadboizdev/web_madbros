          ...
          {isAdmin ? (
            <>
              ...
              <div className={`flex items-center gap-3 pb-3 border-b ${isLight ? 'border-slate-200' : 'border-slate-800'}`}>
                <div className={`p-2.5 rounded-xl border ${
                  isLight ? 'bg-indigo-100 text-indigo-800 border-indigo-200' : 'bg-indigo-500/15 text-indigo-400 border-indigo-500/20'
                }`}>
                  <Mail className="w-5 h-5" />
                </div>
                <div>
                  <h3 className={`text-base sm:text-lg font-bold ${isLight ? 'text-slate-900' : 'text-white'}`}>Cấu Hình Gửi Mail Tự Động (SMTP)</h3>
                  <p className={`text-xs ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>
                    Tự động gửi email thông báo thư mời họp tới toàn bộ nhân sự
                  </p>
                </div>
              </div>
              ...
              <form onSubmit={handleSaveSmtp} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-2.5 rounded-xl text-xs focus:outline-none focus:border-indigo-500 border ${
                ...
                <div>
                  <label className={`block text-xs font-semibold mb-1.5 ${isLight ? 'text-slate-700' : 'text-slate-300'}`}>
                    Địa Chỉ Gmail / Tài Khoản Gửi
                  </label>
                  <input
                    type="email"
                    placeholder="madbros.enterprise@gmail.com"
                    value={smtpUser}
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-2.5 rounded-xl text-xs focus:outline-none focus:border-indigo-500 font-mono border ${
                      isLight ? 'bg-slate-50 border-slate-300 text-slate-900 placeholder-slate-400' : 'bg-slate-900/80 border-slate-700/80 text-white placeholder-slate-500'
                    }`}
                  />
                  <p className="text-[11px] text-slate-500 mt-1">
                    Sử dụng Mật khẩu ứng dụng 16 ký tự do Google cấp (App Passwords) để bảo mật.
                  </p>
                </div>

                <div>
                  <label className={`block text-xs font-semibold mb-1.5 ${isLight ? 'text-slate-700' : 'text-slate-300'}`}>
                    Tên & Email Người Gửi Hiển Thị (From Name)
                  </label>
                  <input
                    type="text"
                    placeholder='"Hệ Thống MadBros" <madbros.enterprise@gmail.com>'
                    value={smtpFrom}
                ...
                <div className="pt-2">
                  <button
                    type="submit"
                    disabled={savingSmtp}
                    className="w-full py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white rounded-xl text-xs font-bold shadow-lg shadow-indigo-600/30 transition flex items-center justify-center gap-2 disabled:opacity-50 hover:scale-[1.01] cursor-pointer"
                  >
              ...
              <div className={`pt-4 border-t space-y-3 ${isLight ? 'border-slate-200' : 'border-slate-800'}`}>
                <h4 className={`text-xs font-bold ${isLight ? 'text-slate-800' : 'text-slate-200'}`}>Gửi Thư Thử Nghiệm (Test Email)</h4>
                {testEmailResult && (
                  <div
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className={`flex-1 px-3 py-2 rounded-xl text-xs focus:outline-none focus:border-indigo-500 border ${
                      isLight ? 'bg-slate-50 border-slate-300 text-slate-900 placeholder-slate-400' : 'bg-slate-900/80 border-slate-700/80 text-white placeholder-slate-500'
                    }`}
                  />
                  <button
                    ...
                    className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition flex items-center gap-1.5 disabled:opacity-50 cursor-pointer"
                  >
                    <Send className="w-3.5 h-3.5" /> {testingEmail ? 'Đang gửi...' : 'Gửi Test'}
                  </button>
                </div>
              </div>
            </div>
